...
TEST_F(VoiceTest, NoteOffDeactivates) {
    voice_.NoteOn(60, 1.0f, 10.0f, 100.0f);
    voice_.NoteOff();
    // Note off triggers release, but voice may still be active briefly
    EXPECT_EQ(voice_.note(), -1);
}
...
TEST_F(VoiceTest, InactiveVoiceProducesSilence) {
    float left[256], right[256];
    ...
    // Inactive voice should zero the buffer
    float sumLeft = 0.0f;
    for (int i = 0; i < 256; ++i) {
        sumLeft += std::abs(left[i]);
    }

    EXPECT_FLOAT_EQ(sumLeft, 0.0f) << "Inactive voice should produce silence";
}
